...
class KNN:
        ...
        for i in range(len(features)):
            ...
            for j in neighbors:
                if j not in count.keys():
                    ...
                    count[j] += 1
            predict_labels.append(max(count, key=count.get))
    ...
    def get_k_neighbors(self, point):
        ...
        for i in range(len(self.training_data)):
            distance_between = self.distance_function(point,self.training_data[i])
            neighbors_dist.append(distance_between)
        k_neighbors = np.argsort(neighbors_dist)
        k_neighbors=k_neighbors[:self.k]
        k_labels = []
        for i in k_neighbors:
            k_labels.append(self.label_data[i])
        return k_labels
